...
1. Drop `apply_lut.py` and your `.cube` LUT file into the same folder as your `.MP4` files.
...
```
  ...
  [2]  H.265  — smaller files (~40%), plays on Pixel / Google Photos
```
...
| | H.264 | H.265 |
|---|---|---|
| Encode speed | Faster | ~2× slower |
| Output size | Larger | ~30–40% smaller |
| Compatibility | Universal | Android 5+, iOS 11+, modern players |
| CRF | 18 | 24 |

H.265 is recommended if your target device supports it (Pixel phones, Google Photos, VLC, modern TVs all handle it fine).
...
- Output is always 8-bit `yuv420p` for maximum device compatibility. 10-bit output is intentionally not used.
